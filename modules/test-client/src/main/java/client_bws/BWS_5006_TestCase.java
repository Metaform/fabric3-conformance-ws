...
/**
 * Client for BWS_5006_TestCase Tests that where an SCA service with a bidirectional interface is invoked by a Web services client, that a callback operation
 * invocation follows the rules defined in Section 3.3 of the WS-Addressing 1.0 - Core specification
 * <p/>
 * - deals with case where there are reference parameters on forward call
 */
public class BWS_5006_TestCase extends BWS_JAXWSTestRunner {
    ...
    protected BWSTestConfiguration getBWSTestConfiguration() {
        ...
        config.output[0] = "BWS_5006 service1 operation1 invoked callback service5Callback callback1 invoked headers OK";
    ...
    public String invokeTest(String input) throws Exception {
        ...
        try {
            // Invoke service5 providing some reference parameters
            String props = UUID.randomUUID().toString();
            scaProps = props;
            scaPropsRequired = true;
            ...
            return "BWS_5006 " + responseValue;
        } catch (SOAPException e) {
            e.printStackTrace();
            return "BWS_5006 service5 operation invocation failed";
        } // end try
    ...
    private String getResponseText(SOAPMessage response) throws SOAPException {
        try {
            ...
            throw new SOAPException("BWS_5006 - exception while handling response to service5 invocation: ", e);
        }
        return null;
    } // end method getResponseText  

} // end class Test_BWS_5006
