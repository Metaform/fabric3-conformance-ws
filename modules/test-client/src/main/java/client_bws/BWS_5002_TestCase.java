...
/**
 * Client for BWS_5002_TestCase Tests that where an SCA service with a bidirectional interface is invoked by a Web services client, that the Web service message
 * sent to the target service contains a Callback EPR, either in the wsa:From SOAP header or in the wsa:ReplyTo header, which the service is able to use to
 * invoke an operation on the callback interface
 */
public class BWS_5002_TestCase extends BWS_JAXWSTestRunner {
    ...
    protected BWSTestConfiguration getBWSTestConfiguration() {
        ...
        config.output[0] = "BWS_5002 service1 operation1 invoked callback service5Callback callback1 invoked";
    ...
    public String invokeTest(String input) throws Exception {
        ...
        try {
            ...
            return "BWS_5002 " + responseValue;
        } catch (SOAPException e) {
            e.printStackTrace();
            return "BWS_5002 service5 operation invocation failed";
        } // end try
    ...
    private String getResponseText(SOAPMessage response) throws SOAPException {
        try {
            ...
            throw new SOAPException("BWS_5002 - exception while handling response to service5 invocation: ", e);
        }
        return null;
    } // end method getResponseText    

} // end class Test_BWS_5002
